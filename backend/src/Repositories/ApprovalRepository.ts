...
import { IProviderRegistration, IProviderRegistrationUrls } from "../Interfaces/Provider/SignIn";
...
class ApprovalRepository implements IApprovalRepository {
    ...
    async providerApprovalRegistration(data: IProviderRegistrationUrls): Promise<boolean> {
        try {
            const id = new mongoose.Types.ObjectId(data.provider_id);
            const service_id = new mongoose.Types.ObjectId(data.expertise_id);
